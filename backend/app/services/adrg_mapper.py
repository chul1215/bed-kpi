...
class ADRGMapper:
    ...
    # ICD-10 매핑 제거됨 (SMC 파일에 ICD-10 코드 없음)
    ...
    def get_adrg_code(self, diagnosis: str) -> Optional[str]:
        """
        진단명으로 ADRG 코드 조회

        우선순위:
        1. 진단명 수동 매핑
        2. 진단명 직접 일치 (ADRG명과 동일)
        3. 진단명 부분 일치 (ADRG명에 포함)
    ...
    def add_adrg_to_smc(
        ...
        # ADRG 코드 매핑 (진단명만 사용)
        df['adrg_code'] = df['diagnosis'].apply(self.get_adrg_code)
