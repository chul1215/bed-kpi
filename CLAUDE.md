...
- **로컬 실행**: `python3 -m http.server 8000` → http://localhost:8000
  - `fetch()` 사용으로 `file://` 직접 열기 불가 (CORS), 반드시 HTTP 서버 필요
...
- 필터: **의사별 진단 환자수 ≥ 6명**
- 대전: 99,377일 / 121,180일 = **86.0%** (cur_occ, data.js 기준)
...
단축 방향 (대전):
  diff > 0 → 줄여야 함 (빨간색 −표시)  → needAdj: diff_off>0.05 || diff_norm>0.05
```
...
```
index.html            # UI + 로직 (~64KB)
├── <style>           # CSS 변수 기반 다크 테마
│   └── 색상 변수: --dj(파랑), --ys(초록), --bg, --card, --hosp-color 등
├── <nav>             # 병원 탭 전환 (대전/유성)
├── <div#p-dj>        # 대전 패널 (JS 동적 생성)
├── <div#p-ys>        # 유성 패널 (JS 동적 생성)
├── <div#doc-modal>   # 의사별 리포트 모달 오버레이
...
└── <script>          # 전체 로직
    ...
    │   └── --hosp-color CSS 변수 주입 (input focus 색상 병원별 적용)
    ...
    ├── openDoc()     # 의사별 리포트 모달 열기
    ├── closeDoc()    # 모달 닫기 (ESC / 오버레이 클릭 / 닫기 버튼)
    ├── dlReport()    # 의사별 MD 보고서 파일 다운로드
    ├── dlSummary()   # 총괄요약 MD 보고서 파일 다운로드
...
reports/              # 의사별 KPI 분석 보고서 MD 파일
├── 대전/             # 42명 + 00_총괄요약.md
│   └── {의사명}_{진료과코드}_KPI분석보고서.md
└── 유성/             # 50명 + 00_총괄요약.md
    └── {의사명}_{진료과코드}_KPI분석보고서.md
...
| `direction` | `"단축"` \| `"연장"` |
| `doc_dept` | `{의사명: 진료과코드}` 딕셔너리 (드롭다운 그룹화 + 보고서 파일명 생성) |
...
의사명 클릭 → `openDoc(h, doctor)` → `#doc-modal` 오버레이 표시

**모달 섹션 순서:**
1. **KPI Stats** (5카드): 총 환자수, 평균 재원일수, 병상일 기여율, 조정 필요 진단, 재원일수 순위
2. **Seasonal Performance**: 비수기/통상/연간 현재→목표 비교 테이블
3. **Top 7 Diagnoses LOS**: SVG 바 차트 (현재 | 연간목표 | 심평원)
4. **Diagnosis Detail**: 진단별 상세 테이블 (비수기/통상/연간/심평원 비교)
5. **AI Analysis & Tasks**: AI 평가 텍스트 + 개선 과제 체크리스트
6. **Strategic Banner**: 목표 달성 시 병상일 추가 확보량
7. **Footer**: 닫기 버튼 + 📄 보고서 저장(MD) 버튼

**보고서 저장 (`dlReport`):**
- 경로: `reports/{병원}/{의사명}_{진료과코드}_KPI분석보고서.md`
- `doc_dept`에 없는 의사: `{의사명}_KPI분석보고서.md` (밑줄 이중 방지)
- fetch → Blob → 브라우저 다운로드
- **주의**: `file://` 프로토콜 불가, HTTP 서버 필요

**총괄요약 저장 (`dlSummary`):**
- 위치: 각 병원 패널 헤더 우측 버튼
- 경로: `reports/{병원}/00_총괄요약.md`
- 다운로드 파일명: `{병원}선병원_00_총괄요약.md`
...
| 대전 테마 | `--dj` | `#137fec` (파랑) |
| 유성 테마 | `--ys` | `#10b981` (초록) |
| 병원별 동적 색상 | `--hosp-color` | render() 에서 패널별 주입 |
| 비수기 | `--indigo` | `#818cf8` |
| 통상기간 | `--gold` | `#f59e0b` |
| 줄여야 함 | `--red` | `#fa6238` |
| 늘려야 함 | `--green` | `#0bda5b` |
...
2. **direction**: 대전 `"단축"`, 유성 `"연장"`. 방향 바뀌면 needAdj, pill 색상, diffColor, vs_hira 색상 로직 모두 영향
3. **심평원 매칭**: 548/548건 완료. 신규 진단 추가 시 `data/hira/입원일수_20260224163001.xlsx`에서 수동 매핑
4. **null 정렬 처리**: `sDir>0 ? 9999 : -9999`로 치환
5. **외부 의존성 없음**: CDN, 라이브러리 미사용. 순수 Vanilla JS + data.js (Google Fonts/Material Symbols CDN 제외)
6. **데이터 분리**: RAW 데이터는 `data.js`로 분리 (GitHub 렌더링 제한 대응)
7. **gp() 버그 수정됨**: 페이지 이동 시 af(h) 선호출로 병원 교차 오염 방지
8. **sw() 버그 수정됨**: 탭 전환 시 curPg=1 리셋 + af(h) 호출
9. **diffColor 방향 반영**: 연장 → diff<0 빨강, 단축 → diff>0 빨강
10. **totalBedGain**: `Math.abs()` 처리로 단축/연장 모두 양수 표시
11. **보고서 파일명**: `{의사명}_{진료과코드}_KPI분석보고서.md` — doc_dept 코드와 1:1 매칭 완료
12. **.nojekyll**: 루트에 존재. GitHub Pages Jekyll 처리 방지
...
- [x] ~~진료과별 드롭다운 그룹화~~ — 완료
- [x] ~~의사별 KPI 리포트 모달~~ — SVG 차트, 시즌별 테이블, AI 평가 포함
- [x] ~~보고서 저장 버튼~~ — 의사별 MD + 총괄요약 MD 다운로드

### 미완료
- [x] ~~CSV 내보내기~~ — 현재 필터 기준, BOM 포함 Excel 호환, 파일명에 모드·날짜 자동 포함
